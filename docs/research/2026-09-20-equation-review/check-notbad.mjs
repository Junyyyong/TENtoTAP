...
const output=path.dirname(new URL(import.meta.url).pathname), revision='16d7856';
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'tapten-notbad-before-'));
...
try{
 ...
 for(const side of ['before','after']){
  const server=await createServer({root:side==='before'?tmp:process.cwd(),configFile:false,server:{host:'127.0.0.1',port:5199,strictPort:true,fs:{allow:[tmp,process.cwd()]}},logLevel:'error'});await server.listen();

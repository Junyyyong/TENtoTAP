...
const output=path.dirname(new URL(import.meta.url).pathname), revision='ffa7ca9';
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'tapten-fixed-grades-before-'));
...
try{
 ...
 for(const side of ['before','after']){
  ...
  try{
   ...
   await page.addInitScript(()=>{localStorage.setItem('makezero.settings.v1',JSON.stringify({musicOn:false,soundOn:false}));let n=20260920;Math.random=()=>{n=(Math.imul(n,1664525)+1013904223)>>>0;return n/4294967296;};});
   ...
   if(side==='after') {
     ...
     checks.push('Actual 30 tutorial stages played','Fixed GOOD TRY/GREAT/AMAZING/UNBELIEVABLE/OH MY GOD at 5/13/22/29/30','All five videos decoded','Stage 29 video leads to CLEAR ALL BLOCKS at 30','Stage 30 leads to 60-second main round');
   }

...
const output=path.dirname(new URL(import.meta.url).pathname), revision='6eec0dc';
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'tapten-resume-before-'));
...
try{
 ...
 for(const side of ['before','after']){
  ...
  try{
   ...
   await page.addInitScript(()=>{localStorage.setItem('makezero.settings.v1',JSON.stringify({musicOn:false,soundOn:false}));let n=20260923;Math.random=()=>{n=(Math.imul(n,1664525)+1013904223)>>>0;return n/4294967296;};});
   ...
   for(let stage=1;stage<=30;stage++){
     ...
     if(stage===10){
       await shot(page,side+'-stage10-before-close');
       await page.reload({waitUntil:'networkidle'});
       await page.clock.runFor(8000);
       await shot(page,side+'-menu-after-reload');
       await click('#mode-timeAttack');await shot(page,side+'-resume-selection');
       await click('#btn-intro-start');
       assert.equal(await page.locator('#stat-c-value').innerText(),side==='after'?'10':'1');
       await shot(page,side+'-resumed-game');
       if(side==='before')break;
     }
     ...
     if((side==='after'?[5,13,22,29,30]:[5,13,22,30]).includes(stage)){
       if(side==='after') assert.equal(await page.locator('#cheer-word').textContent(),({5:'GOOD TRY!',13:'GREAT!',22:'AMAZING!',29:'UNBELIEVABLE!!',30:'OH MY GOD~!'})[stage]);
       assert.equal(await page.locator('#cheer').evaluate(e=>!e.classList.contains('hidden')),true);
       await page.clock.runFor(2100);
       await page.waitForFunction(()=>{const v=document.querySelector('#cheer-clip');return v.videoWidth>0&&v.currentTime>.05;},null,{polling:100,timeout:15000});
       await shot(page,side+'-clear'+stage);
       await click('#cheer');
     }
   }
   if(side==='after') {
     assert.match(await page.locator('#lesson-intro-title').innerText(),/IN 60 SECONDS/);
     assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('makezero.progress.v1')).learningStage),31);
     await page.reload({waitUntil:'networkidle'});await page.clock.runFor(8000);
     await click('#mode-timeAttack');await shot(page,'after-completed-selection');
     await click('#btn-intro-start');
     assert.match(await page.locator('#lesson-intro-title').innerText(),/IN 60 SECONDS/);
     assert.equal(await page.locator('#stat-b-value').innerText(),'0');
     assert.equal(await page.locator('#stat-a-value').innerText(),'01:00');
     await shot(page,'after-completed-resume');
     checks.push('Actual progress at stage 10 survives reload','Launch still shows normal menu and mode selection','Tutorial completion persists as 31; next session starts a fresh main round','Actual 30 tutorial stages played','Fixed GOOD TRY/GREAT/AMAZING/UNBELIEVABLE/OH MY GOD at 5/13/22/29/30','All five videos decoded','Stage 29 video leads to CLEAR ALL BLOCKS at 30','Stage 30 leads to 60-second main round');
   }

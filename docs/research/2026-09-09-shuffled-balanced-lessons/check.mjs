...
const output=path.dirname(new URL(import.meta.url).pathname), revision='757f1e2';
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'tapten-shuffled-balanced-before-'));
 ...
 for(const side of ['before','after']){
  ...
  try{
   ...
   if(side==='after'){
    ...
    checks.push('All nine tiles use distinct colors');

...
const output=path.dirname(new URL(import.meta.url).pathname), revision='7904407';
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'tapten-thirty-lessons-before-'));
  ...
  try{
   ...
   if(side==='before'){
    ...
    for(let stage=1;stage<=30;stage++){
      ...
      assert.equal(await page.locator('#notice').innerText(),count?'Use '+count+' blocks to make 10.':'Clear all blocks.');
      ...
      if(stage===6){
        const wrong=await page.locator('#board .tile').evaluateAll(es=>[es.findIndex(e=>e.textContent==='1'),es.findIndex(e=>e.textContent==='9')]);
        for(const i of wrong){await page.locator(`#board .tile[data-i="${i}"]`).tap({force:true});await page.clock.runFor(50);}
    ...
    checks.push('All 30 tutorial stages played with real taps','Exact guidance on first two stages of each count','Wrong-size ten rejected at stage 6','Stage 30 requires multiple clears','Bonuses only after 5/13/22/30 and timers paused','Stage 31 reached');
   }

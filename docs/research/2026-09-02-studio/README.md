# 2026-09-02 — 스튜디오 오프닝과 화면 배경

## 문제·개선 요구

브랜드 오프닝 도입 후 Safari 상하단에 주황색이 남는 문제가 있었다.

## 무엇을 왜 수정했는가

스튜디오 카드만 주황색으로 칠하고 루트·테마색을 종이색으로 고정했다.

## 수정 후 변화

페이지 배경 처리가 바뀌었다. Chromium의 페이지 캡처에는 실제 Safari 주소창이 없으므로 그 개선을 이미지로 직접 검증할 수 없다.

## 전후 모바일 화면

| 이전 86ee58d | 이후 4870f0a |
|---|---|

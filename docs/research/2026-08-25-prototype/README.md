...
숫자를 선택해 지우는 게임의 첫 실행 가능한 버전이 필요했다.

## 무엇을 왜 수정했는가

같은 숫자 두 개 또는 합계 10의 연결 조합, 탭·드래그, 힌트와 숫자 추가를 구현했다.

## 수정 후 변화

도구 구성만 있던 상태에서 실제 플레이 가능한 MAKEZERO로 바뀌었다.

## 전후 모바일 화면

| 이전 1bded92 | 이후 9af818d |
|---|---|
| 재현 불가: 이 커밋은 빌드 도구만 있으며 index.html과 실행 가능한 게임 화면이 없다. | <img src="after.png" width="260" alt="after" /> |

...
const output=path.dirname(new URL(import.meta.url).pathname), revision='bfc4abe';
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'tapten-mixed-distractors-before-'));
 ...
 for(const side of ['before','after']){
  ...
  try{
   ...
   if(side==='before'){
    ...
    checks.push('All 30 tutorial stages played with real taps','Exact guidance on first two stages of each count','No scale transform during stage transitions','Stage 30 requires multiple clears','Bonuses only after 5/13/22/30 and timers paused','Stage 31 reached');
   }
